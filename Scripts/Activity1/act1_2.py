...
def clean(df: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    ...
    # 2. Quality filter (skipped — no quality flag in this dataset)
    ...
    # 3. Drop metadata / admin columns (none expected, but honour config)
    ...
    # 6. Trim extreme outliers in target column (hs)
    ...
    # 7. Derive scalar wind speed from u/v components
    if {"uwnd", "vwnd"}.issubset(df.columns):
        df[cfg.wind_speed_col] = np.sqrt(df["uwnd"] ** 2 + df["vwnd"] ** 2)
        print(f"  Derived '{cfg.wind_speed_col}' from uwnd / vwnd")
...
def analyse_features(df: pd.DataFrame, cfg: Config) -> None:
    ...
    # — Pairplot (target + top correlated features) —
...
def _pairplot(
    ...
    g.figure.suptitle("Feature pairplot (plasma = significant wave height)", y=1.01, fontsize=13, fontweight="bold")
...
def analyse_distributions(df: pd.DataFrame, cfg: Config) -> None:
    ...
    if target in df.columns:
        ...
        ax.set_title("Distribution of significant wave height (Hs)",
        ...
        sns.despine(fig=fig)
        fig.tight_layout()
        _save("hs_distribution", cfg.output_dir)

    # — Hourly mean Hs with confidence band —
    if "time" in df.columns and target in df.columns:
        ...
        ax.plot(hourly["hour"], hourly["mean"], marker="o", markersize=6,
                linewidth=2, color="#1a3a6b", label="Mean Hs")
        ax.set_title("Mean Hs by hour of day (UTC)",
                     fontsize=14, fontweight="bold", pad=10)
        ax.set_xlabel("Hour of day", fontsize=11)
        ax.set_ylabel("Mean Hs (m)", fontsize=11)
        ax.set_xticks(range(0, 24, 2))
        ax.legend(fontsize=10)
        sns.despine(fig=fig)
        fig.tight_layout()
        _save("hs_by_hour", cfg.output_dir)

    # — Wave partition breakdown (phs0/1/2) —
    partition_cols = [c for c in ["phs0", "phs1", "phs2"] if c in df.columns]
    if partition_cols:
        fig, ax = plt.subplots(figsize=(9, 5))
        for col in partition_cols:
            sns.kdeplot(df[col].dropna(), ax=ax, fill=True, alpha=0.35, label=col)
        ax.set_title("Partition significant wave heights (phs0 / phs1 / phs2)",
                     fontsize=14, fontweight="bold", pad=10)
        ax.set_xlabel("Wave height (m)", fontsize=11)
        ax.set_ylabel("Density", fontsize=11)
        ax.legend(fontsize=10)
        sns.despine(fig=fig)
        fig.tight_layout()
        _save("partition_hs_distribution", cfg.output_dir)

    # — Wind speed distribution —
    if cfg.wind_speed_col in df.columns:
        fig, ax = plt.subplots(figsize=(9, 5))
        sns.histplot(df[cfg.wind_speed_col].dropna(), bins=50, kde=True, ax=ax,
                     color="#2ca02c", alpha=0.55,
                     line_kws={"linewidth": 2.2, "color": "#1a5c1a"})
        median_ws = df[cfg.wind_speed_col].median()
        ax.axvline(median_ws, color="#e05c2a", linewidth=1.8, linestyle="--",
                   label=f"Median {median_ws:.2f} m/s")
        ax.set_title("Distribution of wind speed (derived from uwnd / vwnd)",
                     fontsize=14, fontweight="bold", pad=10)
        ...
        sns.despine(fig=fig)
        fig.tight_layout()
        _save("wind_speed_distribution", cfg.output_dir)

    # — Wind direction rose (uwnd vs vwnd scatter) —
    if {"uwnd", "vwnd"}.issubset(df.columns):
        fig, ax = plt.subplots(figsize=(7, 7))
        sc = ax.scatter(
            df["uwnd"], df["vwnd"],
            c=df[target] if target in df.columns else None,
            cmap="plasma", s=4, alpha=0.4, linewidths=0,
        )
        if target in df.columns:
        ...
        ax.set_title("Wind components (coloured by Hs)",
                     fontsize=14, fontweight="bold", pad=10)
        ax.set_xlabel("U-wind (m/s)", fontsize=11)
        ax.set_ylabel("V-wind (m/s)", fontsize=11)
        sns.despine(fig=fig)
        fig.tight_layout()
        _save("wind_components", cfg.output_dir)
...
def analyse_spatial(df: pd.DataFrame, cfg: Config) -> None:
    ...
    if cfg.target_col in df.columns:
        ...
        cbar = fig.colorbar(sc, ax=ax, pad=0.02, shrink=0.85)
        cbar.set_label("Hs (m)", fontsize=11)
        cbar.ax.tick_params(labelsize=9)

        ax.set_title("Spatial distribution of significant wave height (Hs)",
                     fontsize=14, fontweight="bold", pad=10)
        ax.set_xlabel("Longitude", fontsize=11)
        ax.set_ylabel("Latitude", fontsize=11)
        ax.tick_params(labelsize=9)
        fig.tight_layout()
        _save("spatial_hs", cfg.output_dir)

    # — Scatter map coloured by wind speed —
    if cfg.wind_speed_col in df.columns:
        fig, ax = plt.subplots(figsize=(13, 7))
        fig.patch.set_facecolor("#d6e8f5")
        ax.set_facecolor("#d6e8f5")
        ...
        sc = ax.scatter(
            df["longitude"], df["latitude"],
            c=df[cfg.wind_speed_col], cmap="YlOrRd",
            s=4, alpha=0.6, linewidths=0,
            vmin=df[cfg.wind_speed_col].quantile(0.02),
            vmax=df[cfg.wind_speed_col].quantile(0.98),
        )
        cbar = fig.colorbar(sc, ax=ax, pad=0.02, shrink=0.85)
        cbar.set_label("Wind speed (m/s)", fontsize=11)
        cbar.ax.tick_params(labelsize=9)

        ax.set_title("Spatial distribution of wind speed",
                     fontsize=14, fontweight="bold", pad=10)
        ax.set_xlabel("Longitude", fontsize=11)
        ax.set_ylabel("Latitude", fontsize=11)
        ax.tick_params(labelsize=9)
        fig.tight_layout()
        _save("spatial_wind_speed", cfg.output_dir)

    # — Folium interactive heatmap (Hs) —
    sample = df.sample(
